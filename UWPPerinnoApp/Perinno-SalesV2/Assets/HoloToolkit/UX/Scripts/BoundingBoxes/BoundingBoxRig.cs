...
namespace HoloToolkit.Unity.UX
{
    ...
    public class BoundingBoxRig : MonoBehaviour
    {
        ...
        private void Start()
        {
            scaleHandleMaterial = Resources.Load<Material>("BoundingBoxHandle");
            rotateHandleMaterial = Resources.Load<Material>("BoundingBoxHandle");
            InteractingMaterial = Resources.Load<Material>("BoundingBoxHandleGrabbed");
            BoundingBoxPrefab = Resources.Load<BoundingBox>("BoundingBoxBasic");
            objectToBound = this.gameObject;
            if (BoundingBoxPrefab != null)
            {
                boxInstance = Instantiate(BoundingBoxPrefab) as BoundingBox;
               // Debug.Log("#############Boubdibg Box Prefab null########");
            }
            else
            {
                //Debug.Log("*******Boubdibg Box Prefab frm resources************");
                BoundingBox prefabBox = Resources.Load<BoundingBox>("BoundingBoxBasic");
                boxInstance = Instantiate(prefabBox);
            }
            ...
            //    Debug.Log("#############App bar Box Prefab null########");
            //}
            //else
            //{
            //    Debug.Log("*******AppBar Box Prefab frm resources************");
            //    AppBar prefabApp = Resources.Load<AppBar>("AppBar");
            //    appBarInstance = Instantiate(prefabApp);
            //}
        ...
        private GameObject BuildRig()
        {
            ...
            Debug.Log("INFO: Object to Bound:" + objectToBound.name);
            Debug.Log("INFO: Object to Bound Scale:" + scale);
            GameObject rig = new GameObject();
            rig.name = "center";
            rig.transform.SetPositionAndRotation(Vector3.zero, Quaternion.identity);
            rig.transform.localScale = new Vector3(1.0f / scale.x, 1.0f / scale.y, 1.0f / scale.z);
            Debug.Log("INFO: Rig Local scale: " + rig.transform.localScale);
