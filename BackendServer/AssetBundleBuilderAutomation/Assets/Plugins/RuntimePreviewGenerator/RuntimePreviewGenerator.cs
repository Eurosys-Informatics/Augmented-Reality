...
public static class RuntimePreviewGenerator
{
    ...
    public static Texture2D GenerateModelPreviewWithShader(Transform model, Shader shader, string replacementTag, int width = 64, int height = 64, bool shouldCloneModel = false)
    {
        ...
        try
        {
            ...
            //Debug.Log("INFO : Rnder Texture temp format is :" + temp);
            RenderTexture renderTex = RenderTexture.GetTemporary(width, height, 16, RenderTextureFormat.ARGB32);
            bool createTex = renderTex.Create();
            if (createTex)
            {
                Debug.Log("INFO: Render Texture created");
            }
            else
            {
                Debug.Log("INFO: Render Texture is not created");
            }
            Debug.Log("INFO : Render Texture is :" + renderTex);
            ...
            renderCamera.targetTexture = new RenderTexture(renderTex); ;
            Debug.Log("INFO :Target Texture format is :" + renderCamera.targetTexture.format);

            if (SystemInfo.SupportsRenderTextureFormat(renderTex.format))
            {
                Debug.Log("INFO :Target Texture format is :" + renderTex.format + " : is supported");
            }
            else
                Debug.Log("INFO :Target Texture format is :" + renderTex.format + " : is *not* supported");
            bool targetTexCreate = renderCamera.targetTexture.Create();
            if (targetTexCreate)
            {
                Debug.Log("INFO: Render Camera target texture created");
            }
            RenderTexture.active = renderCamera.targetTexture;
            if (shader == null)
            {
                Debug.Log("INFO: Shader is null. Calling renderCamera.Render() with target texture format: " + renderCamera.targetTexture.format);
                renderCamera.Render();
            }
            else
            ...
            result = new Texture2D(width, height, m_transparentBackground ? TextureFormat.ARGB32 : TextureFormat.ARGB32, false);
