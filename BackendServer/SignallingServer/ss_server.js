...
winston.info('Listning on port 80');
...
io.on('connection', function (socket) {
    ...
    socket.on('credentials', function (user, email, photo_url) {
        ...
        iplocate(clientIp.substring(7), ['http://api.ipstack.com/*?access_key=3456086fc3caaed350516949a876b718'] , function (error, res) {
			var mailtext = '<p> Username:' + user + '</p>' + '<p> Email:' + email + '</p>' + ' <p> Photo URL:' + photo_url + '</p>' + ' <p> IP address:' + clientIp + '</p>' + ' <p> IP Geo:' + JSON.stringify(res, null, '\t') + '</p>';
            var mailOptions = {
            ...
            if (!(email == 'amitpsingh@gmail.com' || email == 'rohitru@gmail.com' || email == 'amiacs@gmail.com' || user == 'perinno_hololens'|| email == 'user@ios.com')) {
